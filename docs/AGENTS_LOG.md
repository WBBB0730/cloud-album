...
这个文件归档历史执行记录，避免 PLAN.md 变成流水账。通常不需要逐轮读取；只有需要追溯历史改动、验证记录或上下文恢复细节时再查。
...
- `docs/design/index.html` 已同步。
- `pnpm.cmd typecheck` 通过。
- 新建空间入口图标进一步改为 lucide `HousePlus`，`docs/design/index.html` 已同步，`pnpm.cmd typecheck` 通过。
...
## 2026-05-05 应用名称
- `src/app/layout.tsx` 的 `metadata.title`、`metadata.applicationName` 和 Apple Web App 标题统一改为“云相册”。
- `public/manifest.webmanifest` 的 `name` 和 `short_name` 统一改为“云相册”。
- 已扫描确认源码、public 和 docs 中不再残留 `Cloud Album`。
- `pnpm.cmd typecheck` 通过。
- 应用名进一步改为“拾云”，描述改为“一个轻量的云相册”；同步更新 layout metadata、manifest、登录页标题、设计稿标题和 PRD/技术设计标题。已扫描确认无旧英文名、旧描述和旧标题残留，`pnpm.cmd typecheck` 通过。

## 2026-05-05 PWA Maskable 图标
- 使用用户生成的 `public/icons/icon-192-maskable.png` 和 `public/icons/icon-512-maskable.png` 作为 maskable 图标。
- `public/manifest.webmanifest` 将普通图标标为 `purpose: "any"`，新增 maskable 图标并标为 `purpose: "maskable"`。
- `public/manifest.webmanifest` 增加 SchemaStore `$schema`，用于编辑器校验；浏览器会忽略该字段。
- `src/app/layout.tsx` 同步补充 maskable 图标入口。
- 已检查 maskable 图标尺寸分别为 192x192 和 512x512，且无透明像素。
- `pnpm.cmd typecheck` 通过。
- 已删除临时生成的 `public/icons/maskable-192.png` 和 `public/icons/maskable-512.png`。

## 2026-05-05 协作约定
- 已将“如果文件内容和我上次修改后不一样，优先视为用户手动修改，不擅自覆盖或回退”写入 `MEMORY.md`。
