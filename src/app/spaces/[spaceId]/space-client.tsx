    ...
    <MobileFrame className="ca-scroll-layout">
      <div className="ca-fixed-section">
        <TopBar
          ...
          actions={
            <>
              ...
              <Link href={`/spaces/${spaceId}/trash`} className="ca-icon-btn" aria-label="回收站">
                <Trash2 />
              </Link>
            </>
          }
        ...
        <div className="ca-view-switch" aria-label="视图切换">
          <Link href={`/spaces/${spaceId}?view=grid`} className={`ca-switch-btn ${view === "grid" ? "active" : ""}`} aria-label="卡片视图">
            <Grid2X2 className="size-4" />
          </Link>
          <Link href={`/spaces/${spaceId}?view=list`} className={`ca-switch-btn ${view === "list" ? "active" : ""}`} aria-label="列表视图">
      ...
      <div className="ca-scroll-section">
        ...
        {data ? (
          data.folders.length === 0 ? (
            <EmptyState title="还没有相册">创建一级相册后开始上传图片和视频。</EmptyState>
          ) : view === "grid" ? (
