          ...
          data.folders.length > 0 ? (
            <div className="ca-list">
              {data.folders.map((folder) => {
                const restoreAction = restoreFolderAction.bind(null, spaceId, folder.id)
                const permanentAction = permanentFolderAction.bind(null, spaceId, folder.id)

                return (
                  <div key={folder.id} className="ca-folder-row">
                    <Link href={`/spaces/${spaceId}/trash/${folder.id}`} className="ca-row-cover ca-photo-a">
                      {folder.coverUrl ? <SafeImage src={folder.coverUrl} alt="" sizes="56px" /> : null}
                    </Link>
                    <Link href={`/spaces/${spaceId}/trash/${folder.id}`} className="min-w-0">
                      <strong>{folder.name}</strong>
                      <small>
                        {folder.deletedByName} · {formatDateTime(folder.deletedAt)} · {folder.itemCount} 项
                      </small>
                    </Link>
                    <div className="flex gap-1">
                      ...
                      <form action={permanentAction}>
                        <button className="ca-icon-btn text-red-600" aria-label="永久删除">
            ...
            <EmptyState title="回收站为空">删除的相册和媒体会按原相册出现在这里。</EmptyState>
          )
        ) : null}
      </div>
    </MobileFrame>
