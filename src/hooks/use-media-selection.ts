...
export function useMediaSelection(orderedIds: string[]) {
  ...
  const getDragRangeIds = useCallback(
  ...
  const applyDragSelectionRange = useCallback(
    (currentId: string) => {
  ...
  const runDragAutoScroll = useCallback(
    function tick() {
      ...
      if (topDistance >= 0 && topDistance < DRAG_SELECT_SCROLL_EDGE) {
        ...
        bottomDistance >= 0 &&
        bottomDistance < DRAG_SELECT_SCROLL_EDGE
      ) {
        scrollDelta = Math.ceil(
          ((DRAG_SELECT_SCROLL_EDGE - bottomDistance) /
            DRAG_SELECT_SCROLL_EDGE) *
            DRAG_SELECT_MAX_SCROLL_SPEED
        )
      }
  ...
  const beginDragSelect = useCallback(
    (mediaId: string, select: boolean, clientX: number, clientY: number) => {
      ...
      applyDragSelectionRange(mediaId)
      dragSelectRef.current.scrollFrame =
        window.requestAnimationFrame(runDragAutoScroll)
    },
    [
      applyDragSelectionRange,
      runDragAutoScroll,
      selectedIds,
      stopDragAutoScroll,
    ]
  ...
  const getMediaPointerHandlers = useCallback(
